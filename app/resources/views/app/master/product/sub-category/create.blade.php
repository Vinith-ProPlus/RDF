                    ...
                    <div class="row">
                        <div class="col-sm-12">
                            <div class="form-group">
                                ...
                                <input type="text" class="form-control {{$Theme['input-size']}}" id="txtPSCName" value="<?php if($isEdit==true){ echo $EditData[0]->PSCName;} ?>" autocomplete="off">
...
<script>
    $(document).ready(function(){
        ...
        const getCategory=async()=>{
            let categoryTypeId = ($('#lstPCategoryType').val() !== "") ? $('#lstPCategoryType').val() : $('#lstPCategoryType').attr('data-selected');
            ...
            if(categoryTypeId !== "") {
                ...
                $.ajax({
                    type: "post",
                    url: "{{url('/')}}/admin/master/product/category/get/PCategory?PCTID=" + categoryTypeId,
                    headers: {'X-CSRF-Token': $('meta[name=_token]').attr('content')},
                    dataType: "json",
                    async: true,
                    beforeSend:async()=>{
                        $('#btnReloadPCategoryIcon').addClass('fa-spin');
                    },
                    ...
                    complete: function (e, x, settings, exception) {
                        setTimeout(() => {
                            $('#btnReloadPCategoryIcon').removeClass('fa-spin');
                        }, 500);
                    },
                    success: function (response) {
                        for (let Item of response) {
                            let selected = "";
                            if (Item.PCID == $('#lstPCategory').attr('data-selected')) {
                                selected = "selected";
                            }
        ...
        const formValidation=async()=>{
            ...
            if(PCTID===""){
                $('#lstPCategoryType-err').html('The Product Category Type Name is required.');status=false;
            }
            if(Category==""){
        ...
        $(document).on('change','#lstPCategoryType',function(){
            getCategory();
        });
        $(document).on('click','#btnReloadPCategoryInSC', function() {
            getCategory();
        });
