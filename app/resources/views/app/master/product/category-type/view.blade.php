				...
				<div class="card-header text-center">
					<div class="row">
						...
						<div class="col-12 col-sm-4 my-2 text-right">
							@if($crud['restore']==1)
								<a href="{{ url('/') }}/admin/master/product/category-type/trash" class="btn  btn-outline-dark {{$Theme['button-size']}} mr-10" type="button" > Trash </a>
							@endif
							@if($crud['add']==1)
								<a href="{{ url('/') }}/admin/master/product/category-type/create" class="btn  btn-outline-success btn-air-success {{$Theme['button-size']}}" type="button" >Create</a> <!-- full-right -->
							@endif
						</div>
					</div>
				</div>
    ...
    $(document).ready(function(){
        const LoadTable=async()=>{
			...
			$('#tblCategory').dataTable( {
				"bProcessing": true,
				"bServerSide": true,
                "ajax": {"url":"{{url('/')}}/admin/master/product/category-type/data?_token="+$('meta[name=_token]').attr('content'),"headers":{ 'X-CSRF-Token' : $('meta[name=_token]').attr('content') } ,"type": "POST"},
				deferRender: true,
				responsive: true,
				dom: 'Bfrtip',
				"iDisplayLength": 10,
		...
		$(document).on('click','.btnEdit',function(){
			window.location.replace("{{url('/')}}/admin/master/product/category-type/edit/"+ $(this).attr('data-id'));
		});
		...
		$(document).on('click','.btnDelete',function(){
            ...
            function(){swal.close();
            	$.ajax({
            		type:"post",
                    url:"{{url('/')}}/admin/master/product/category-type/delete/"+ID,
