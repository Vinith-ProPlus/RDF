				...
				<div class="card-header text-center">
					<div class="row">
						...
						<div class="col-12 col-sm-4 my-2 text-right">
							@if($crud['restore']==1)
								<a href="{{ url('/') }}/admin/master/product/coupon/trash" class="btn  btn-outline-dark {{$Theme['button-size']}} mr-10" type="button" > Trash </a>
							@endif
							@if($crud['add']==1)
								<a href="{{ url('/') }}/admin/master/product/coupon/create" class="btn  btn-outline-success btn-air-success {{$Theme['button-size']}}" type="button" >Create</a> <!-- full-right -->
							@endif
						</div>
					</div>
				</div>
				<div class="card-body " >
					<div class="row">
						<div class="col-12 col-sm-12">
							<table class="table {{$Theme['table-size']}}" id="tblCoupon">
								<thead>
									<tr>
    ...
    $(document).ready(function(){
        const LoadTable=async()=>{
			...
			$('#tblCoupon').dataTable( {
				"bProcessing": true,
				"bServerSide": true,
                "ajax": {"url":"{{url('/')}}/admin/master/product/coupon/data?_token="+$('meta[name=_token]').attr('content'),"headers":{ 'X-CSRF-Token' : "{{ csrf_token() }}" } ,"type": "POST"},
				deferRender: true,
				responsive: true,
				dom: 'Bfrtip',
				"iDisplayLength": 10,
		...
		$(document).on('click','.btnEdit',function(){
			window.location.replace("{{url('/')}}/admin/master/product/coupon/edit/"+ $(this).attr('data-id'));
		});
		...
		$(document).on('click','.btnDelete',function(){
			...
			swal({
                title: "Are you sure?",
                text: "You want Delete this Coupon!",
                type: "warning",
            ...
            function(){swal.close();
            	$.ajax({
            		type:"post",
                    url:"{{url('/')}}/admin/master/product/coupon/delete/"+ID,
