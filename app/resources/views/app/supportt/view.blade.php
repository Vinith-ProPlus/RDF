				...
				<div class="card-header text-center">
					<div class="row">
						...
						<div class="col-sm-4 my-2 text-right text-md-right">
							@if($crud['restore']==1)
								<a href="{{ url('/') }}/admin/support/trash" class="btn  btn-outline-dark {{$Theme['button-size']}} m-r-10" type="button" > Cancelled Tickets </a>
							@endif
							@if($crud['add']==1)
								<a href="{{ url('/') }}/admin/support/create" class="btn  btn-outline-success btn-air-success {{$Theme['button-size']}}" type="button" >Create</a>
							@endif
						</div>
					</div>
				</div>
					...
					<div class="row">
						<div class="col-12 col-sm-12 col-lg-12">
                            <table class="table" id="tblTicket">
                                <thead>
                                    <tr>
                                        <th class="text-center align-middle">Ticket ID</th>
                                        <th class="text-center align-middle">Ticket Date</th>
										<th class="text-center align-middle">Vendor / Customer</th>
										<th class="text-center align-middle">User Name</th>
                                        <th class="text-center align-middle">Mobile Number</th>
                                        <th class="text-center align-middle">Support Type</th>
                                        <th class="text-center align-middle">Ticket Status</th>
    ...
    $(document).ready(function(){
        ...
        const LoadTable=async()=>{
			...
			$('#tblTicket').dataTable( {
				"bProcessing": true,
				"bServerSide": true,
                "ajax": {"url": "{{url('/')}}/admin/support/data?_token="+$('meta[name=_token]').attr('content'),"headers":{ 'X-CSRF-Token' : $('meta[name=_token]').attr('content') } ,"type": "POST"},
				deferRender: true,
				responsive: true,
				dom: 'Bfrtip',
				"iDisplayLength": 10,
				...
				columnDefs: [
					{"className": "dt-center", "targets":[5,6,7,8]},
				]
			});
			@endif
        }
		$(document).on('click','.btnView',function(){
			...
			$.ajax({
            		type:"post",
                    url:"{{url('/')}}/admin/support/details/"+ID,
                    headers: { 'X-CSRF-Token' : $('meta[name=_token]').attr('content') },
                    dataType:"json",
                    success: function (response) {
						let modalContent = $('<div></div>');
						let row1 = $('<div class="row my-3 justify-content-center"></div>').html(
							`<div class="row">
								<div class="col-sm-4 mt-20">
									<div class="form-group">
										<label>Subject </label>
										<input disabled type="text" class="form-control {{$Theme['input-size']}}" value="${response.Subject}">
									</div>
								</div>
								<div class="col-sm-12 mt-20">
									<div class="form-group">
										<label>Description</label>
										<textarea disabled class="form-control" name="">${response.Description}</textarea>
									</div>
								</div>
							</div>`
						);
						...
						if (response.Attachments.length > 0) {
							modalContent.append(`<div class="row my-3 justify-content-center">
													<h6 class="text-center">Attachments</h6>
												</div>`);
							let row2 = $(`<div class="row my-3 justify-content-center">
										</div>`);
							let images = '';
							response.Attachments.slice(0, 3).forEach(function (attachment) {
								images += `<div class="col-sm-4">
												<img src="{{ url('/') }}/${attachment.UploadUrl}" class="img-fluid rounded" alt="Attachment">
										</div>`;
							});
							row2.html(images);
		...
		$(document).on('click','.btnSolved',function(){
			let ID=$(this).attr('data-id');
			swal({
                title: "Are you sure?",
                text: "You want to Mark this Ticket as Solved!",
                type: "warning",
            ...
            function(){swal.close();
            	$.ajax({
            		type:"post",
                    url:"{{url('/')}}/admin/support/solved/"+ID,
                    headers: { 'X-CSRF-Token' : $('meta[name=_token]').attr('content') },
                    dataType:"json",
                    success:function(response){
                    	swal.close();
