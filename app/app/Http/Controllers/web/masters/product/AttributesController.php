...
class AttributesController extends Controller{
    ...
    public function edit(Request $req, $AttrID)
    {
        if ($this->general->isCrudAllow($this->CRUD, "edit")) {
            ...
            if (count($FormData['EditData']) > 0) {
                foreach ($FormData['EditValuesData'] as $index => $EditValuesData){
                    $FormData['EditValuesData'][$index]->valuesInTranslation = json_decode($EditValuesData->valuesInTranslation);
                }
                return view('app.master.product.attributes.create', $FormData);
            } else {
                return view('errors.403');
				...
				if($status){
					foreach($Values->ExistingValues as $row){
						...
						$data=[
                            ...
                            "valuesInTranslation"=>$row->valuesInTranslation,
							"ValueLogo"=>$ValueLogo,
				...
				if($status){
					...
					foreach($Values->ExistingValues as $row){
						if($row->ValueID){
                            ...
                            $oldAttDetail = DB::Table('tbl_attributes_details')->where('AttrID',$AttrID)->where('ValueID',$row->ValueID)->first();
                            $newTranslations = json_decode($row->valuesInTranslation);
                            $existingTranslations = json_decode($oldAttDetail->valuesInTranslation);
                            ...
                            $data['valuesInTranslation'] = json_encode($existingTranslations);
	...
	public function ActiveStatus(Request $req,$AttrID){
		...
		if($this->general->isCrudAllow($this->CRUD,"delete")){
			...
			try{
				...
				DB::table('tbl_attributes')->where('AttrID',$AttrID)->update(array("ActiveStatus"=>$req->ActiveStatus,"UpdatedBy"=>$this->UserID,"UpdatedOn"=>date("Y-m-d H:i:s")));
                DB::commit();
	...
	public function Delete(Request $req,$AttrID){
		...
		if($this->general->isCrudAllow($this->CRUD,"delete")){
			...
			try{
				$OldData=DB::table('tbl_attributes')->where('AttrID',$AttrID)->get();
				DB::table('tbl_attributes')->where('AttrID',$AttrID)->update(array("DFlag"=>1,"DeletedBy"=>$this->UserID,"DeletedOn"=>date("Y-m-d H:i:s")));
                DB::commit();
    ...
    public function Restore(Request $req, $AttrID)
    {
        ...
        if ($this->general->isCrudAllow($this->CRUD, "restore")) {
            ...
            try {
                $OldData = DB::table('tbl_attributes')->where('AttrID', $AttrID)->get();
                DB::table('tbl_attributes')->where('AttrID', $AttrID)->update(array("DFlag" => 0, "UpdatedBy" => $this->UserID, "UpdatedOn" => date("Y-m-d H:i:s")));
                DB::commit();
