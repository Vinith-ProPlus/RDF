$(document).ready(function(){
    ...
    const validatePCategoryType=async()=>{
        $('.errors.new-category').html('');
        let status=true;
    ...
    const getPCategoryType=async(elem)=>{
        $('#'+elem).select2('destroy');
        $('#'+elem+' option').remove();
        $('#'+elem).append('<option value="" selected>Select a Product Category Type</option>');
        $.ajax({
            type:"post",
            url:RootUrl+"admin/master/product/category-type/get/PCategoryType",
            beforeSend:async()=>{
                $('#btnReloadPCategoryType i').addClass('fa-spin');
            },
            headers: { 'X-CSRF-Token' : $('meta[name=_token]').attr('content') },
            dataType:"json",
            async:true,
            ...
            complete:async()=>{
                setTimeout(() => {
                    $('#btnReloadPCategoryType i').removeClass('fa-spin');
                }, 500);
            },
            success:function(response){
                for(let Item of response){
                    let selected="";
                    if(Item.PCTID==$('#'+elem).attr('data-selected')){selected="selected";}
                    $('#'+elem).append('<option '+selected+' value="'+Item.PCTID+'">'+Item.PCTName+' </option>');
                }
            }
        });
        $('#'+elem).select2();
    ...
    const createPCategoryType=async(elem)=>{
        $.ajax({
            type:"post",
            url:RootUrl+"admin/master/product/category-type/create-form",
            headers: { 'X-CSRF-Token' : $('meta[name=_token]').attr('content') },
            data:{},
            async:true,
            dataType:"html",
            ...
            success:function(response){
                bootbox.dialog({
                    title: 'Create New Product Category Type',
                    closeButton: true,
                    message: response,
                    className:"dynamicValueModal",
                    buttons: {
    ...
    $('#btnReloadPCategoryType').click(function(){
        let id=$(this).closest('div').find('select').attr('id');
        getPCategoryType(id);
    });
    $('#btnAddPCategoryType').click(function(){
        let id=$(this).closest('.row').find('select').attr('id');
        createPCategoryType(id);
    });
    $(document).on('click','#btnCreatePCategoryType',async function(){
        ...
        if(status===true){
            ...
            swal({
                    title: "Are you sure?",
                    text: "Do you want add this Product Category Type",
                    type: "warning",
                    showCancelButton: true,
                    confirmButtonClass: "btn-success",
                    confirmButtonText: "Yes, Add it!",
                ...
                async function(){
                    swal.close();
                    btnLoading($('#btnCreatePCategoryType'));
                    $.ajax({
                        type:"post",
                        url:RootUrl+"admin/master/product/category-type/create",
                        headers: { 'X-CSRF-Token' : $('meta[name=_token]').attr('content') },
                        data:formData,
                        async:true,
                        dataType:"json",
                        ...
                        error:function(e, x, settings, exception){ajaxErrors(e, x, settings, exception);},
                        complete: function(e, x, settings, exception){btnReset($('#btnCreatePCategoryType'));ajaxIndicatorStop();},
                        success:function(response){
                            if(response.status==true){
                                toastr.success(response.message, "Success", {
                                    positionClass: "toast-top-right",
                                ...
                                if( $('#btnReloadPCategoryType').length>0){
                                    $('#btnReloadPCategoryType').trigger('click');
                                }
                            }else{
                                toastr.error(response.message, "Failed", {
                                    positionClass: "toast-top-right",
                                ...
                                if(response['errors']!=undefined){
                                    ...
                                    $.each( response['errors'], function( KeyName, KeyValue ) {
                                        var key=KeyName;
                                        if(key=="PCTName"){$('#txtMPCTName-err').html(KeyValue);}
                                        if(key=="PCTImage"){$('#txtMPCTImage-err').html(KeyValue);}
                                    });
                                }
                            }
                        }
    ...
    const validatePCategory = async () => {
        $('.errors.new-category').html('');
        let status = true;
        let PCTID = $('#lstMPCategoryType').val();
        let PCName = $('#txtMPCName').val();

        if (PCTID === "") {
            $('#lstMPCategoryType-err').html('The Product Category Type is required.');
            status = false;
        }
        ...
        if (PCName == "") {
            ...
            $('#txtMPCName-err').html('Product Category Name must be greater than 2 characters');
            status = false;
    ...
    const validatePSubCategory = async () => {
        ...
        if (PSCName === "") {
            $('#txtMPSCName-err').html('The Product Sub Category Name is required.');
            status = false;
        } else if (PSCName.length < 2) {
            $('#txtMPSCName-err').html('Product Sub Category Name must be greater than 2 characters');
            status = false;
        } else if (PSCName.length > 100) {
            $('#txtMPSCName-err').html('Product Sub Category Name may not be greater than 100 characters');
            status = false;
        }
        if (PCTID === "") {
            $('#lstMPCategoryType-err').html('The Product Category Type is required.');
